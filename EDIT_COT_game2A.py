...
def load_map():
    ...
    with open("levels2c.pkl", "rb") as file:
        layout = pickle.load(file)
    return layout

layout = load_map()

def save_map(levels="levels2c.pkl"):
    with open("levels2c.pkl", "wb") as file:
...
fx_save = pygame.mixer.Sound("assets/save.ogg")
tile_chosen_number = 0
tile = pygame.Surface((0, 0))
clock = pygame.time.Clock()


def modify_map(tile, sound):
  x, y = get_pos()
  if layout[room][y][x] != " ":
