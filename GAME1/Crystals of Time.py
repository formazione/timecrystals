        ...
        if player.x + 32 > self.x and player.x < self.x + 32 and not self.col:
            if player.y + 32 > self.y and player.y + 32 < self.y + 16:
                ...
                # 4 is jump
                    ...
                    player.ySpeed = -10
                    player.bottomCol = False # ============== FALSE BOTTOM
                    self.y += 200
                    pygame.mixer.Sound.play(sfx_crystal)
                ...
                player.y = self.y + 32
                player.ySpeed = 0
                player.topCol = True # =============================== TOP
                self.col = True
        
        # COLLISION WITH TILE

        on_top = player.y + 32 > self.y and player.y < self.y + 32 and not self.col
        hit_right = player.x + 32 > self.x and player.x + 32 < self.x + 16
        hit_left = player.x > self.x + 16 and player.x < self.x + 32
        if on_top:

            if hit_right:
                player.x = self.x - 32
                # self.y -= 32 # makes the tile on the right levitate
                player.xSpeed = -0.4
                player.rightCol = True # it hits the tile ont the right
                self.col = True
...
class Crystal():
    ...
    def draw(self):
        if not self in remove:
            ...
            display.blit(ball1, ((int(self.x) + math.cos(timer / 16) * 16), int(self.y) + math.sin(timer / 16) * 16), ((timer % 16 < 8) * 32, 0, 32, 32))
            display.blit(ball2, ((int(self.x) - math.cos(timer / 16) * 16), int(self.y) + math.cos(timer / 16) * 16), ((timer % 16 < 8) * 32, 0, 32, 32))
class LargeCrystal():
    def __init__(self, x, y):
        self.x = x
        self.y = y
