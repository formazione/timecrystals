...
spr_player = load("images/player.png").convert_alpha()
# ======================= IMAGES ==============================
...
spr_tiles = load("images/tiles4.png").convert_alpha()
NUM_OF_TILES = spr_tiles.get_size()[0] // 32
spr_crystal1 = load("images/crystal.png").convert_alpha()
spr_crystal2 = load("images/crystal2.png").convert_alpha()
spr_particle = load("images/particles.png").convert_alpha()
spr_number = load("images/number.png").convert_alpha()
background = load("images/background.png").convert()
title = load("images/title.png").convert()

# ==================== SOUNDS ========================
sfx_crash = pygame.mixer.Sound("sounds/crash.wav")
sfx_crash.set_volume(0.2)
sfx_collect = pygame.mixer.Sound("sounds/collect.wav")
...
class Crystal():
    def __init__(self, x, y, num):
        ...
        self.y = y
        self.num = num
        self.random = random.randint(8,32)
    def update(self):
        global countdown
    ...
    def draw(self):
        if not self in remove:
            screen.blit(spr_crystal1,(int(self.x), int(self.y) + math.sin(timer*3 / 32) * self.random), ((timer % 16 < 8) * 32, 0, 32, 32))
            screen.blit(ball1, ((int(self.x) + math.cos(timer / 32) * 16), int(self.y) + math.sin(timer / 16) * 16), ((timer % 32 < 8) * 32, 0, 32, 32))
            screen.blit(ball1, ((int(self.x) + math.cos(timer / 16) * 16), int(self.y) + math.sin(timer / 16) * 16), ((timer % 32 < 8) * 32, 0, 32, 32))
            # screen.blit(ball1, ((int(self.x) + math.sin(timer / 32) * 16), int(self.y) + math.cos(timer / 16) * 16), ((timer % 16 < 16) * 32, 0, 32, 32))
            # screen.blit(ball1, ((int(self.x) - math.cos(timer / 8) * 2), int(self.y) + math.cos(timer / 16) * 2), ((timer % 16 < 8) * 32, 0, 32, 32))
            # screen.blit(ball1, ((int(self.x) - math.cos(timer / 8) * self.random//2), int(self.y) + math.sin(timer / 16) * 32), ((timer % 16 < 4) * 32, 0, 32, 32))
            # screen.blit(ball1, ((int(self.x) - math.sin(timer / 8) * self.random//2), int(self.y) + math.sin(timer / 16) * 16), ((timer % 16 < 8) * 32, 0, 32, 32))
...
while run:
    ...
    for i in range(room_r):
        for j in range(room_c): # ================= Put the plauer in position
            ...
            #     load.append(player)
                # ========================================== Here go the tiles
            if layout[room_num][i][j] > -1 and layout[room_num][i][j] < 100:
    ...
    if player.x + 16 < 0: # if player goes back...
        player_y = player.y # 
        player_x = 480 - 24 # player in room - 1 at the end
        room_num -= 1
    if room_num < room_len - 1: # if it is not in the last room
        if player.x + 16 > 480: # if it's at the end of the room
            player_y = player.y # the height is the same
            player_x = -8 # but player is at the start of the screen
            room_num += 1 # the room is the next
    else:
        player_x = 480 - 32 # if it's in the last room, it does not go next
